...
var username = flag.String("username", "hivetool", "Matrix username localpart")
...
func main() {
	...
	client, err := matrix.Dial(*username)
	if err != nil {
		panic(err)
	}
	defer client.Close()
	...
	log.Info().
		Str("homeserver_url", client.HomeserverURL.String()).
		Str("user_id", client.UserID.String()).
		Str("device_id", client.DeviceID.String()).
		Str("access_token", client.AccessToken).
		Msg("Connected")
